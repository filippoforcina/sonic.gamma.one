...
public class QueueBasket extends AbstractConsumer {

	private static final String TYPE = Router.BASKET.name();
	...
	// @RabbitListener(queues = "${queue.basket}")
	...
	@RabbitAnnotation
	@RabbitListener(id = "${queue.basket}", queues = "${queue.basket}", containerFactory = "rabbitListenerManualAckContainerFactory", autoStartup = "false")
